 ...
 */
import {
  tasksDisplay, addTask, removeTask, localstorage, editButton, checkBtn,
} from '../__mocks__/functionsMock.js';
...
describe('edit a task in the list', () => {
  test('should change description from Task 2 to New Task', () => {
    const taskA = localstorage.getItem(0);
    const taskAIndex = localstorage.getItem(0).index;
...
describe('complete property of a task in a list', () => {
  test('change all checked item\'s completed status to true', () => {
    ...
    const id1 = taskA.getAttribute('id');
    const id2 = taskC.getAttribute('id');
